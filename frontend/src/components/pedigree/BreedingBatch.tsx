...
import { Plus, ChevronLeft, Swords, Calendar, Save, Users, History, Trash2, ArrowRight, CheckCircle, Heart, AlertTriangle, Edit, X, Hash, Archive, Crown, Home, Layers } from 'lucide-react';
...
export default function BreedingBatch({ onNavigate }: { onNavigate: (page: any) => void }) {
  ...
  const [filterSource, setFilterSource] = useState<'active' | 'farm' | 'vip' | 'archived'>('active');
  ...
  const filteredBatches = batches.filter(batch => {
    const isArchived = batch.isArchived === true;
    const isVip = batch.mother?.source === 'ไก่ฟาร์มอื่น (ลูกค้า VIP)';
    
    // Check Filter Type
    if (filterSource === 'archived' && !isArchived) return false;
    if (filterSource !== 'archived' && isArchived) return false; // Hide archived in other tabs
  ...
  const handleToggleArchive = async (batch: any) => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/breeding-batches/${batch._id}`, {
        method: 'PUT',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}` 
        ...
        body: JSON.stringify({ 
          ...
          father: batch.father?._id || batch.father,
          mother: batch.mother?._id || batch.mother,
          breedingDate: batch.breedingDate,
          notes: batch.notes,
      ...
      if (res.ok) {
        ...
        setAlertConfig({
          show: true,
          title: 'สำเร็จ',
          message: batch.isArchived ? 'นำชุดการผสมกลับมาใช้งานแล้ว' : 'จัดเก็บชุดการผสมเรียบร้อยแล้ว',
          type: 'success'
        });
      } else {
        ...
        setAlertConfig({
          show: true,
          title: 'เกิดข้อผิดพลาด',
          message: data.message || 'ไม่สามารถทำรายการได้',
          type: 'error'
        });
      }
    } catch (err) {
      ...
      <div className="max-w-6xl mx-auto w-full p-4 flex-1 space-y-4">
        ...
        <div className="grid grid-cols-4 gap-1.5 sm:gap-2 mb-4">
          <button
            ...
            className={`flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-1.5 px-1 sm:px-3 py-2 sm:py-2.5 text-[10px] sm:text-xs font-black rounded-xl transition-all duration-300 ${filterSource === 'active' ? 'bg-gradient-to-r from-blue-500 to-indigo-500 text-white shadow-md shadow-blue-500/20' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800'}`}
          >
            <Layers className="w-4 h-4 sm:w-3.5 sm:h-3.5" /> <span>ปัจจุบัน</span>
          </button>
          <button
          ...
          <button
            onClick={() => setFilterSource('archived')}
            className={`flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-1.5 px-1 sm:px-3 py-2 sm:py-2.5 text-[10px] sm:text-xs font-black rounded-xl transition-all duration-300 ${filterSource === 'archived' ? 'bg-gradient-to-r from-slate-600 to-slate-800 text-white shadow-md shadow-slate-500/20' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800'}`}
          >
            <Archive className="w-4 h-4 sm:w-3.5 sm:h-3.5" /> <span>จัดเก็บ</span>
          </button>
        </div>
            ...
            {filteredBatches.map((batch) => (
              <div key={batch._id} className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-700/50 overflow-hidden shadow-md hover:shadow-lg hover:border-blue-200 dark:hover:border-blue-800 transition-all duration-300">
                <div className="bg-slate-50 dark:bg-slate-800/80 px-4 py-3 flex justify-between items-center border-b border-slate-200 dark:border-slate-700/50">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button 
                      ...
                      title={batch.isArchived ? "นำกลับมา" : "จัดเก็บ"}
                      className={`p-1.5 transition-colors rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700/50 ${batch.isArchived ? 'text-blue-500 hover:text-blue-600' : 'text-slate-400 hover:text-orange-500'}`}
                    >
                ...
                <div className="p-4">
                  ...
                  <div className="flex flex-row items-center justify-between text-xs text-slate-500 border-t border-slate-100 dark:border-white/5 pt-3 mt-2">
                    <div className="flex items-center gap-2">
                      <button 
                        onClick={() => handleToggleArchive(batch)}
                        className={`font-bold px-2 py-1.5 rounded-lg flex items-center gap-1 transition-colors ${batch.isArchived ? 'text-orange-600 bg-orange-50 hover:bg-orange-100' : 'text-emerald-600 bg-emerald-50 hover:bg-emerald-100'}`}
                      >
                        <CheckCircle className="w-3.5 h-3.5 shrink-0" />
                        <span>{batch.isArchived ? 'นำกลับมา' : 'เสร็จสิ้น'}</span>
                      </button>
                      <div className="flex items-center gap-1 text-slate-400 whitespace-nowrap">
                        <Calendar className="w-3.5 h-3.5 shrink-0" />
                        <span>{new Date(batch.breedingDate).toLocaleDateString('th-TH')}</span>
                      </div>
                    </div>
                    <button 
                      onClick={() => onNavigate('chick-registry', /^[a-fA-F0-9]{24}$/.test(batch.batchCode || '') ? undefined : batch.batchCode)}
                      className="text-blue-600 font-bold hover:text-blue-700 bg-blue-50 hover:bg-blue-100 px-2 sm:px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1 shrink-0 whitespace-nowrap"
                    >
                      ดูลูกไก่ <ArrowRight className="w-3 h-3 shrink-0" />
                    </button>
                  </div>
                </div>
              </div>
