      ...
      <div className="relative w-full max-w-3xl flex flex-col items-center h-full">
        ...
        <div className="w-full flex justify-between items-center mb-4 shrink-0">
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-6 py-2.5 font-bold rounded-xl shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
              downloadSuccess 
          ...
          >
            ...
            {downloadSuccess ? 'บันทึกสำเร็จ!' : downloading ? 'กำลังสร้างใบเซอร์...' : 'บันทึกภาพ (Download)'}
          </button>
        </div>
        ...
        >
          ...
          <div style={{ height: `${1123 * scale}px`, width: `${794 * scale}px`, position: 'relative' }}>
            ...
            <div 
              ref={certificateRef}
              className="w-[794px] h-[1123px] bg-slate-900 relative overflow-hidden rounded-sm shadow-2xl flex flex-col border-[12px] border-slate-800 origin-top-left absolute top-0 left-0"
              style={{ 
                ...
                <div className="flex items-center justify-between w-full bg-slate-800/20 py-2 px-6 rounded-3xl border border-slate-700/30 shadow-lg">
                  <div className="flex items-center gap-8">
                    ...
                    <div className="flex flex-col items-start justify-center">
                      ...
                      <h3 className={`${(chicken.name || '').length > 15 ? 'text-xl md:text-2xl' : 'text-3xl'} font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-300 mb-1.5 drop-shadow-md leading-tight line-clamp-2 break-words max-w-[350px]`}>
                        {chicken.name}
                      </h3>
                      {chicken.code && (
                  ...
                  <div className="flex flex-col items-center justify-center relative mt-2 pr-4">
                    {chicken.user?.signatureImage ? (
                      <img src={chicken.user.signatureImage} alt="Signature" className="h-12 object-contain drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]" />
                    ) : (
                      <div className="text-3xl text-amber-400 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] -rotate-6 pb-1" style={{ fontFamily: "'Charm', cursive" }}>
                    ...
                    {chicken.user?.stampText !== '' && (
                      <div className="absolute top-1/2 left-1/2 -translate-x-[50%] -translate-y-[60%] w-20 h-20 border-2 border-red-500/70 rounded-full flex items-center justify-center -z-10 rotate-12 bg-red-500/5 mix-blend-screen shadow-[0_0_15px_rgba(239,68,68,0.2)]">
                        <div className="text-red-500/70 text-[9px] font-black uppercase text-center leading-tight tracking-widest px-2 drop-shadow-sm">
